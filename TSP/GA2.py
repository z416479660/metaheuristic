# reference: https://zhuanlan.zhihu.com/p/344588977
# 这个效果不好，可以理解它的交叉算子实现
...
from tools import time_it, GetData
...
def selection(routes, fitness_values, best_route):
    """
    选择操作
    :param routes: 所有路线 ndarray
    ...
    for i in range(n_routes):
        choice = np.random.choice(range(n_routes), p=probability)  #这里进行了轮盘赌选择
        selected_routes[i] = routes[choice]
    # region 保留最好的
    selected_routes[0] = best_route
    # end
...
if __name__ == '__main__':
    ...
    cities_, CityNum, Dist = GetData('../data/TSP25cities.tsp')
    ...
    for i_ in range(epoch):
        routes_ = selection(routes_, fitness_values_, best_route)  # 选择
